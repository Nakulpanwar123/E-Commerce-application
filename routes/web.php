...
Route::prefix('storefront-data')->group(function () {
    Route::get('/bootstrap', [StorefrontApiController::class, 'bootstrap']);
    Route::get('/home', [StorefrontApiController::class, 'home']);
    Route::get('/sale', [StorefrontApiController::class, 'sale']);
    Route::get('/search', [StorefrontApiController::class, 'search']);
    Route::get('/category/{slug}', [StorefrontApiController::class, 'category']);
    Route::get('/product/{slug}', [StorefrontApiController::class, 'product']);
    Route::get('/cart', [StorefrontApiController::class, 'cart']);
    Route::get('/blogs', [StorefrontApiController::class, 'blogs']);
    Route::get('/blogs/{slug}', [StorefrontApiController::class, 'blog']);
});

// Search
Route::get('/search', StorefrontController::class)->name('search');
Route::get('/search/suggest', [SearchController::class, 'suggest'])->name('search.suggest');

// Static pages
Route::get('/size-guide', StorefrontController::class)->name('size-guide');

// Sale
Route::get('/sale', StorefrontController::class)->name('sale');

// Blog
Route::prefix('blog')->name('blog.')->group(function () {
    Route::get('/', StorefrontController::class)->name('index');
    Route::get('/{slug}', StorefrontController::class)->name('show');
});

// Cart (guest + auth)
Route::prefix('cart')->group(function () {
    Route::get('/', StorefrontController::class)->name('cart');
...
Route::get('/p/{slug}', StorefrontController::class)->name('product.show');

// Category catch-all (SEO-friendly URLs)
Route::get('/{slug}', StorefrontController::class)->name('category')
    ->where('slug', 'men|women|kids|new-arrivals|trending|men/.+|women/.+|kids/.+');

// ── Authenticated ────────────────────────────────────────────────────────────
Route::middleware('auth')->group(function () {
    Route::get('/storefront-data/wishlist', [StorefrontApiController::class, 'wishlist']);
    Route::get('/storefront-data/outfit-builder', [StorefrontApiController::class, 'outfitBuilder']);
    ...
    // Wishlist
    Route::get('/wishlist', StorefrontController::class)->name('wishlist');
    ...
    // Outfit Builder
    Route::get('/outfit-builder', StorefrontController::class)->name('outfit-builder');
