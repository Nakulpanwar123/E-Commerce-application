<article class="product-card" x-data="{ wishlisted: $store.wishlist.has({{ $product->id }}) }">
    <div class="relative aspect-product overflow-hidden bg-gray-100">
        <a href="{{ route('product.show', $product->slug) }}">
            <img
                data-src="{{ $product->primary_image }}"
                data-srcset="{{ $product->primary_image_srcset }}"
                alt="{{ $product->name }}"
                class="w-full h-full object-cover skeleton group-hover:scale-105 transition-transform duration-500"
                width="300" height="400"
            >
            @if($product->discount_percent)
            <span class="absolute top-3 left-3 badge bg-red-500 text-white">-{{ $product->discount_percent }}%</span>
            @endif
            @if($product->is_new)
            <span class="absolute top-3 left-3 badge bg-green-500 text-white">New</span>
            @endif
        </a>
        ...
        {{-- Quick Add --}}
        <div class="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm py-3 px-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
            <button
                class="w-full btn-primary text-sm py-2"
                @click="$store.cart.add({{ $product->id }}, {{ $product->default_variant_id ?? 'null' }})"
            >Quick Add</button>
        </div>
    </div>
    ...
    <div class="p-4">
        <a href="{{ route('product.show', $product->slug) }}">
            ...
            <h3 class="font-medium text-gray-900 line-clamp-1 group-hover:text-primary-600 transition-colors">{{ $product->name }}</h3>
        </a>
